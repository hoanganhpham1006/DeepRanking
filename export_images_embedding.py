...
images_embedding = []
for foldername in tqdm(os.listdir(os.getcwd()+'/image')):
    for filename in os.listdir(os.getcwd()+'/image/'+foldername):
        emb = ge.predict_embedding(os.getcwd()+'/image/'+foldername+'/'+filename)
        d = [0, 0]
        d[0] = 'image/'+foldername+'/'+filename
        d[1] = emb
        images_embedding.append(d)
print(len(images_embedding))
pickle.dump(images_embedding, open("images_embedding.pkl", "wb"))
